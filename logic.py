...
def translate_to_gloss(text):
    ...
    prompt = (
        "You are an ASL (American Sign Language) translator. Convert the English sentence to ASL GLOSS format.\n\n"
        "STRICT RULES:\n"
        "1. Output ONLY single words in ALL CAPS separated by spaces\n"
        "2. Use ASL word order: Time-Topic-Comment (not English order)\n"
        "3. Remove: articles (a, an, the), auxiliary verbs (is, am, are, was, were), prepositions when not needed\n"
        "4. Keep: action verbs, nouns, adjectives, question words\n"
        "5. Use simple, common sign language vocabulary\n"
        "6. NO punctuation, NO extra text, NO explanations\n\n"
        f"English: {text}\n"
        "ASL GLOSS:"
    )
